...
class TaskState {
    ...
    @action.bound
    setRealId(task, id, sort_order) {
        // After creating a task with a fake ID, use this method to set the actual
        // id used in the database and clear out the old data.
        // Leave the temporary_id in place; it's used as a key for the React component
        // that holds the task entry. Removing it can disrupt the user's editing flow.
        // Update the task's sort_order value as well, since that's generated by
        // django-orderable in the backend.
